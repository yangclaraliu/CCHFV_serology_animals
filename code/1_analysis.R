...
pdf(paste0("figures/forest_plot_", species_inuse,".pdf"), width=8.5, height=0.25*nrow(dat_ag))
forest(res_ag, 
...
pdf(paste0("figures/funnel_plot_", species_inuse, ".pdf"), width = 6, height = 6)
...
write_csv(coef_tbl, paste0("results/coefficients_", species_inuse, ".csv"))
write_csv(coef_rob, paste0("results/coefficients_robust_", species_inuse, ".csv"))
...
X <- model.matrix(~ continent + year + bias1 + bias2, data = newdat)
